 ...
 * Org keys require an `X-Project-Id` header to resolve the target project.
 */
...
/**
 * Validate an org API key without requiring `X-Project-Id`.
 * Returns only the userId and organizationId — no project context.
 * Used by endpoints that operate at org level (e.g. provisioning).
 */
