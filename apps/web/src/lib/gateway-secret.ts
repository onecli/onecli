...
/**
 * Get the gateway host address.
 * Uses `GATEWAY_HOST` env var if set, otherwise falls back to Docker internal host
...
export const getGatewayHost = (): string => {
  ...
  // In Docker (OSS), the gateway runs as a sibling container
...
/**
 * Build the base URL for the gateway's HTTP API (e.g. `http://localhost:10255`).
 */
export const getGatewayBaseUrl = (): string => {
  return `http://${getGatewayHost()}:${GATEWAY_PORT}`;
};
