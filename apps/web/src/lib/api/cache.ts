...
// connect cache for the active workspace lets agents pick up secret, rule,
...
// authenticates as the ACTING USER: the Cognito ID token + X-Workspace-Id in
// cloud, session-cookie credentials in OSS. The gateway scopes the flush to
// that authenticated principal's workspace (its AuthUser extractor), so there is
...
/**
 * Flush the gateway cache for the current workspace. Fire-and-forget: a failed
