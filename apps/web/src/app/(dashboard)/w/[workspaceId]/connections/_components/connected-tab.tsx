...
export const ConnectedTab = ({
  ...
  const vaultsQuery = useVaultConnections(pageScope === "workspace");

  const loading =
    connectionsQuery.isPending ||
    secretsQuery.isPending ||
    (pageScope === "workspace" && vaultsQuery.isPending);

  const items = useMemo(() => {
    const connections = connectionsQuery.data ?? [];
    const secrets = secretsQuery.data ?? [];
    const vaults = pageScope === "workspace" ? (vaultsQuery.data ?? []) : [];
    ...
    const secretItems: ConnectedItem[] = secrets.map((s) => {
      ...
      return {
        ...
        scope: s.scope,
        typeLabel: isInherited ? labelForScope(s.scope) : s.typeLabel,
