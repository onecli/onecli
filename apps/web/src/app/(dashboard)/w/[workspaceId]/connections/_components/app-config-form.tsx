...
export const AppConfigForm = ({
  ...
  // workspace row behind it — the credentials live on the organization, and the
  ...
  // org config surface (workspace responses omit `dependents`). Drives the confirm
  // gating and the precise counts in the dialogs; zero on the workspace page, so
  // that flow stays keyed on `isConnected` alone.
  const dependents = statusQuery.data?.dependents;
  const dependentTotal =
    (dependents?.orgConnections ?? 0) + (dependents?.workspaceConnections ?? 0);
  const dependentSentence = (() => {
    ...
    if (dependents.workspaceConnections > 0) {
      parts.push(
        `${dependents.workspaceConnections} workspace ${
          dependents.workspaceConnections === 1 ? "connection" : "connections"
        }`,
      );
    }
  ...
  if (!loading && !openInitialized) {
    ...
    // usable without workspace-level setup, so the section stays collapsed.
          ...
          <Card className="p-4 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                {!hasEnvDefaults && !hasCredentials && !enabled && (
                  <CloudUpsell
                    before="Or connect with"
