  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col gap-6">
      ...
      <StatsCards
        ...
        loading={loading}
      />
      {(resourceCounts.agentsWorkingInBackground ?? 0) > 0 && (
        <p className="text-muted-foreground -mt-3 text-sm">
          {resourceCounts.agentsWorkingInBackground === 1
            ? "1 agent is working in the background right now."
            : `${resourceCounts.agentsWorkingInBackground} agents are working in the background right now.`}
        </p>
      )}
      <RecentActivityCard />
    </div>
