...
//! plaintext data key is zeroed. The shared [`crate::crypto::CryptoService`]
...
use crate::crypto::{aes_gcm_decrypt, check_iv_and_tag, seal_with_key};
...
pub(crate) struct KmsEnvelopeCrypto {
    client: KmsClient,
}
...
impl KmsEnvelopeCrypto {
    ...
    pub(crate) async fn decrypt(&self, parts: &[&str]) -> Result<String> {
    ...
    /// [`crate::crypto::CryptoService::decrypt`].
    pub(crate) async fn encrypt(&self, plaintext: &str) -> Result<String> {
