...
pub(crate) const PLATFORM_SECRET_ID: &str = "platform:anthropic";
...
pub(crate) fn platform_key() -> Option<&'static PlatformKey> {
...
pub(crate) fn configured_for_host(hostname: &str) -> bool {
...
pub(crate) fn pool_has_llm_credential(secrets: &[crate::db::SecretRow]) -> bool {
    secrets.iter().any(|s| {
        matches!(s.type_.as_str(), "anthropic" | "openai")
            || crate::policy::is_llm_host(&s.host_pattern)
    })
}
...
/// key.
pub(crate) async fn platform_credential(
    pool: &PgPool,
    org_id: &str,
    hostname: &str,
    pool_has_llm: bool,
    entitled: bool,
) -> Option<(crate::inject::InjectionRule, BudgetBinding)> {
    if !entitled {
        return None;
    }
    ...
    let rule = crate::inject::InjectionRule {
        path_pattern: "*".to_string(),
        injections: crate::secret_inject::build_injections("anthropic", &key.api_key, None, None),
    };
...
mod tests {
    ...
    fn secret(type_: &'static str, host_pattern: &'static str) -> crate::db::SecretRow {
        crate::db::SecretRow {
            id: "s1".into(),
