...
//!
//! Covered by the OneCLI Enterprise License (see `src/ee/LICENSE`). These are
...
//! `enforce_key_rechecks` (shared, `crate::auth`) enables them — cloud, or a
...
//! `crate::db` — the free product depends on them.
...
pub(crate) async fn user_can_manage_workspace(
    pool: &PgPool,
    user_id: &str,
    workspace_id: &str,
) -> Result<bool> {
...
/// (`crate::ee::rbac::user_can_manage_workspace`, step 14) against a real
/// Postgres.
///
/// The recheck enforces on cloud and licensed self-hosts (see
/// `crate::auth::enforce_key_rechecks`); these tests exercise the SQL itself,
