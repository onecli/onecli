...
pub(crate) trait RequestGuard: Sync {
...
pub(crate) trait TokenScoper: Sync {
...
pub(crate) trait ResourceAxis: Sync {
...
fn guard_for_host(host: &str) -> Option<(&str, &'static dyn RequestGuard)> {
    let host = crate::gateway::strip_port(host);
    let (provider, _) = crate::apps::provider_for_host(host)?;
...
pub(crate) fn denies_everything(policy: Option<&Value>) -> bool {
    let Some(policy) = policy else { return false };
...
pub(crate) fn intersect_policies(a: Option<&Value>, b: Option<&Value>) -> Option<Value> {
...
pub(crate) fn has_token_scoper(cred_type: &str) -> bool {
...
pub(crate) async fn scope_token(
    cred_type: &str,
    creds: &Value,
    policy: Option<&Value>,
