...
const PRINCIPAL_CTE: &str = r#"
        ...
        all_users AS (
            ...
            -- people-gate `crate::ee::rbac::user_can_manage_workspace`.
            SELECT cu.id
...
/// `crate::ee::rbac::user_can_manage_workspace`. Workspace-derived, so every
...
pub(crate) async fn find_principal_set(
    pool: &PgPool,
    workspace_id: &str,
    organization_id: &str,
) -> Result<PrincipalSet> {
...
/// and `crate::policy_engine::loaders::find_direct_user_principals` must stay
...
mod parity_tests {
    ...
    use crate::policy_engine::find_direct_user_principals;
    use sqlx::PgPool;
    ...
    /// idempotent-reset pattern as the `crate::ee::rbac` suite).
