...
//! `crate::approval` / `crate::cache`; the shared `create_store` selection
...
pub(crate) use approval::redis_approval_store;
pub(crate) use cache::redis_cache_store;
...
pub(crate) fn check_ha_entitlement(redis_host: Option<&str>, entitled: bool) -> Result<()> {
    if entitled {
        return Ok(());
    }
