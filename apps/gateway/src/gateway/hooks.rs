...
use crate::ee::response as ee_response;
...
// `RequestMeta::into_event` lives in `telemetry_core` (shared with the budget
// meter, which compiles in every edition).
...
fn call_limit_for_plan(edition: crate::edition::Edition, plan: &str) -> u64 {
    if edition != crate::edition::Edition::Cloud {
        return 0;
    }
    if plan == "free" {
...
pub(crate) fn prepare_request(
    ...
    if rules
        .budget_bindings
        .iter()
        .any(|b| crate::ee::budget::has_meter(&b.secret_type))
    {
...
pub(crate) fn needs_request_body(
    ...
    crate::ee::granular_access::needs_request_body(
...
pub(crate) fn refuse_empty_scope(
    ...
    if !crate::ee::granular_access::denies_everything(rules.session_policy.as_ref()) {
        return None;
    }
...
pub(crate) async fn pre_forward(
    ...
    if let Some(denial) = crate::ee::granular_access::enforce_request(
        rules.session_policy.as_ref(),
        host,
        path,
        headers,
        ...
        warn!(host = %super::strip_port(host), %path, reason = %denial.reason, "granular access denied");
    ...
    let limit = call_limit_for_plan(crate::edition::edition(), &rules.plan);
    ...
    if count > limit && !crate::policy::is_llm_host(host) {
...
pub(crate) fn track_and_wrap(
    ...
    if (200..300).contains(&meta.status) {
        if let Some(binding) = rules
            .budget_bindings
            .iter()
            .find(|b| crate::ee::budget::has_meter(&b.secret_type))
        {
            let is_sse = resp_headers
                .get(hyper::header::CONTENT_TYPE)
                .and_then(|v| v.to_str().ok())
                .is_some_and(|ct| ct.contains("text/event-stream"));
            return crate::ee::budget::wrap_metered(binding, meta, is_sse, Box::pin(stream));
        }
    }
    crate::telemetry::on_request(meta.into_event(None));
...
fn emit_block_telemetry(
    ...
    crate::telemetry::on_request(crate::telemetry::RequestEvent {
        ...
        timestamp: ts,
        injected: false,
        decision: crate::telemetry_core::RequestDecision::Blocked {
