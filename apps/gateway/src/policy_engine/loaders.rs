...
use crate::db::{PolicyRuleV2Row, POLICY_V2_SELECT};

/// Active published organization-scope rules (max published generation),
/// first-match ordered.
pub(crate) async fn find_published_policy_rules_v2_by_org(
    pool: &PgPool,
    organization_id: &str,
...
/// `crate::ee::principals` instead — an unlicensed deployment never expands
/// groups, so group-bound rules cannot match and group-inherited members do
/// not count as user principals.
pub(crate) async fn find_direct_user_principals(
    pool: &PgPool,
    workspace_id: &str,
    organization_id: &str,
) -> Result<Vec<String>> {
    let rows: Vec<(String,)> = sqlx::query_as(
        ...
        // database test beside the CTE (`crate::ee::principals`).
