...
pub fn budget_exceeded(
    binding: &crate::budget::BudgetBinding,
    workspace_id: Option<&str>,
) -> Response<ForwardBody> {
    let limit_usd = binding.limit_nanos as f64 / 1e9;
    let (window_label, period) = match binding.period {
        crate::budget::BudgetPeriod::Monthly => ("this month", "monthly"),
        crate::budget::BudgetPeriod::Total => ("in total", "total"),
    };
    ...
    // matching.
    let is_platform_credit = binding.secret_id == crate::platform_llm::PLATFORM_SECRET_ID;
...
mod tests {
    use super::*;
    use crate::budget::{BudgetBinding, BudgetPeriod, BudgetSubject};
    ...
    fn budget_exceeded_wire_codes_distinguish_org_budget_from_trial_credit() {
        ...
        let platform = body_of(budget_exceeded(
            &BudgetBinding {
                secret_id: crate::platform_llm::PLATFORM_SECRET_ID.into(),
