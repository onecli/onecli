...
//!   GATEWAY_TEST_DATABASE_URL=… cargo test -p policy-engine enforce_pg
use super::assemble::assemble_v2;
use super::enforce::needs_body_buffer;
...
/// The github connection carrying the GraphQL-discrimination grant stack
/// (allow graphql_query + list_pulls; mutations/create_pull in the blocked
/// complement) - also authored via the REAL grants service.
const CONN_GH_GRAPHQL: &str = "gwenf-conn-gh-graphql";
...
async fn enforce_over_seeded_fixtures_closes_the_injection_host_bypass() {
    ...
    let decide = |r: &PolicyRequest| evaluate_new(&rules, r, ConditionBody::None);
    ...
    // The GraphQL-discrimination stack (seeded through the REAL grants
    // service on its own github connection: allow graphql_query + list_pulls,
    // everything else in the blocked complement). Decided with REAL request
    // bodies through the same evaluate path the proxy calls - the end-to-end
    // proof that a Never'd mutation cannot ride the query allow, and that the
    // fail-closed arms hold, over rows production would write.
    let gh_graphql = |body: &[u8]| {
        evaluate_new(
            &rules,
            &req_via("api.github.com", "POST", "/graphql", Some(CONN_GH_GRAPHQL)),
            ConditionBody::Full(body),
        )
    };
    let query_body = br#"{"query": "query { repository(owner: \"o\", name: \"r\") { pullRequests(first: 5) { nodes { title } } } }"}"#;
    let mutation_body = br#"{"query": "mutation { createPullRequest(input: {baseRefName: \"main\"}) { pullRequest { number } } }"}"#;
    let mixed_body =
        br#"{"query": "query Q { viewer { login } } mutation M { createPullRequest(input: {}) { clientMutationId } }"}"#;
    assert_eq!(
        gh_graphql(query_body),
        Decision::allow(),
        "graphql stack: a pure query rides the query allow"
    );
    assert_eq!(
        gh_graphql(mutation_body),
        Decision::block(),
        "graphql stack: a createPullRequest mutation dies on the blocked complement despite the query allow"
    );
    assert_eq!(
        gh_graphql(mixed_body),
        Decision::block(),
        "graphql stack: a mixed query+mutation document fails closed to the block"
    );
    assert_eq!(
        decide(&req_via(
            "api.github.com",
            "POST",
            "/graphql",
            Some(CONN_GH_GRAPHQL),
        )),
        Decision::block(),
        "graphql stack: a bodyless POST /graphql fails closed to the block"
    );
    assert_eq!(
        decide(&req_via(
            "api.github.com",
        ...
        Decision::allow(),
        "graphql stack: the allowed REST tool (list_pulls) still permits"
    );
    assert_eq!(
        decide(&req_via(
            "api.github.com",
        ...
        Decision::block(),
        "graphql stack: create_pull (REST) dies on the blocked complement"
    );
    // The body-buffer signal must ARM for this rule set: without it the proxy
    // would never hand the engine a body and every allowed query would fail
    // closed at the classifier.
    let graphql_v2 = PolicyV2Rules {
        org: org.clone(),
        workspace: workspace.clone(),
        secret_hosts: secret_hosts.clone(),
        connection_providers: providers.clone(),
        ..Default::default()
    };
    assert!(
        needs_body_buffer(&graphql_v2, "api.github.com", "/graphql"),
        "needs_body_buffer must arm for the seeded graphql grant stack"
    );
    // The host-narrowed signal (#999) must NOT arm for unrelated large-bodied
    // traffic through the same workspace — the regression that dragged LLM
    // bodies into the 16KB buffer.
    assert!(
        !needs_body_buffer(&graphql_v2, "api.anthropic.com", "/v1/messages"),
        "needs_body_buffer must stay dark for hosts no body-needing rule can match"
    );
    ...
    assert_eq!(
        evaluate_new(
            &rules_b,
            &req("fence-only.example.com", "GET", "/"),
            ConditionBody::None
        ),
        Decision::block(),
