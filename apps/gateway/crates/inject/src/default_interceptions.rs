...
#[derive(Debug)]
pub struct DefaultInterception {
    /// Exact hostname (no port), e.g. `"auth.openai.com"`.
    host: &'static str,
    /// Path pattern matched via [`crate::path_matches`] (glob-aware).
    path: &'static str,
...
impl DefaultInterception {
    ...
    pub fn handle(&self, body: &[u8]) -> Option<SyntheticResponse> {
        (self.handler)(body)
    }
}
...
pub fn match_target(
    ...
    REGISTRY
        .iter()
        .find(|i| i.host == host && *method == i.method && crate::path_matches(path, i.path))
}
