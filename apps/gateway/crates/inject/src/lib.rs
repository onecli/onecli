...
pub fn is_catch_all_pattern(pattern: &str) -> bool {
    matches!(pattern, "*" | "/*")
}

/// True when any rule's `path_pattern` matches the request path. A missing
/// request path is conservatively non-matching.
pub fn rules_serve_path(rules: &[InjectionRule], request_path: Option<&str>) -> bool {
...
pub fn path_matches(request_path: &str, pattern: &str) -> bool {
...
pub fn vault_credential_to_rules(hostname: &str, cred: &VaultCredential) -> Vec<InjectionRule> {
