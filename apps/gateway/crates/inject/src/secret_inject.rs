...
#[must_use]
pub fn is_oauth_mode(metadata: Option<&serde_json::Value>) -> bool {
    metadata
...
/// Whether a stored host pattern is a shape a CREDENTIAL may be injected on.
///
/// The write-time validator (`hostPatternSchema`) refuses to store a pattern
/// whose tail is a wildcard, because such a pattern names no owner: `*.foo.*`
/// spans `foo.com`, `foo.so`, and equally `foo.xyz`, which anyone can register.
/// This is the same rule enforced at READ time, and it exists because write-time
/// validation only binds rows written after it shipped. Rows that predate it are
/// still in the database, and a matcher change must never silently hand a live
/// credential to a host such a row was never actually injecting on.
///
/// Decision-only rules (allow/block/ask) are unaffected: this gate is on the
/// credential-selection path alone, where the blast radius is a leaked secret
/// rather than a widened permission.
fn is_injectable_host_pattern(pattern: &str) -> bool {
    // A single `*` is the long-standing prefix/suffix form and stays as-is.
    if pattern.bytes().filter(|b| *b == b'*').count() < 2 {
        return true;
    }
    // Multi-wildcard: every label after the last `*` must be literal, and there
    // must be at least one — i.e. the pattern ends by naming a real domain.
    pattern.rsplit('.').next().is_some_and(|last| last != "*")
}
...
pub fn secret_injects_on_host(
    ...
    secret_host_patterns(secret_type, host_pattern, metadata)
        .iter()
        .filter(|p| is_injectable_host_pattern(p))
...
pub async fn refresh_openai_oauth_if_expired(
...
pub async fn find_secret_hosts(
    pool: &PgPool,
    organization_id: &str,
    workspace_id: &str,
) -> Result<SecretHosts> {
    ...
    for row in rows {
        ...
        let patterns = secret_host_patterns(&row.type_, &row.host_pattern, row.metadata.as_ref());
...
mod tests {
    ...
    #[test]
    fn legacy_trailing_wildcard_secret_never_injects() {
        // The regression this guard exists for. A row like `*.notion.*` predates
        // the write-time validator, matches NOTHING under the single-`*`
        // matcher, and must keep injecting nowhere once the matcher learns
        // multi-wildcard patterns — otherwise a deploy would silently start
        // handing a live token to `notion.so`, `notion.com`, and any
        // `notion.<tld>` an attacker can register.
        for host in [
            "api.notion.so",
            "api.notion.com",
            "evil.notion.xyz",
            ...
            assert!(
                !secret_injects_on_host("generic", "*.notion.*", None, host),
                "a trailing-wildcard pattern must never inject (host: {host})"
            );
        }
    }

    #[test]
    fn well_formed_patterns_still_inject() {
        // The guard is narrow: it refuses only the shape that names no owner.
        ...
        assert!(secret_injects_on_host("generic", "*", None, "anything.com"));
        assert!(secret_injects_on_host(
            "generic",
            "api.github.com",
            None,
