...
pub fn redis_host_configured() -> bool {
    std::env::var("REDIS_HOST").is_ok_and(|v| !v.trim().is_empty())
}

/// The in-memory cache store (single-instance deployments, unit tests).
///
/// Backend selection lives in the composition root (`wiring`): Redis when
/// `REDIS_HOST` is set (the licensed multi-instance backend in `ee::ha`),
/// else this store. This module never reaches into the licensed code.
pub fn in_memory() -> std::sync::Arc<dyn CacheStore> {
    std::sync::Arc::new(InMemoryCacheStore::new())
}
