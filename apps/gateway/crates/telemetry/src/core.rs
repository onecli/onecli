...
//! so consumer code uses `crate::*` without change.
...
pub struct RequestEvent {
    ...
    pub matched_rule: Option<policy::MatchedRule>,
}

/// Common telemetry fields for a proxied request, filled by the forward path
/// and consumed by [`RequestMeta::into_event`]. Lives here (not in the gateway
/// hooks) because both the request hooks and the budget meter build events
/// from it.
pub struct RequestMeta {
    ...
    pub connection_label: Option<String>,
    pub existing_log_id: Option<String>,
    pub decision: Option<RequestDecision>,
...
impl RequestMeta {
    ...
    pub fn into_event(self, budget_charge: Option<BudgetCharge>) -> RequestEvent {
        RequestEvent {
            org_id: self.org_id,
...
pub fn spawn_flush_loop<F>(flush_loop: F)
where
...
pub async fn shutdown(deadline: std::time::Duration) {
...
pub fn extract_columns(events: &[&RequestEvent]) -> BatchColumns {
    BatchColumns {
        ids: events
            .iter()
            .map(|e| {
